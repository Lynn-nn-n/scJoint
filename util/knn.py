...
from config import Config  # Assuming Config is a custom module or class for configuration settings

def neighbor_hit_cnt(rna_cnt, neighbor_indexs):
    """Calculate how many times each RNA neighbor is hit."""
...
def compute_scores(knn_label, predictions, neighbor_indexs, topn):
    """Compute confidence scores based on KNN predictions."""
...
def KNN(config, neighbors=30, knn_rna_samples=20000):
    """K-nearest neighbors classification and scoring."""
    ...
    # Subsample RNA data if necessary
    rna_embedding_knn = []
    rna_label_knn = []
    rna_prediction_knn = []
    ...
    # Calculate hit counts and confidence scores
    ...
    # Test accuracy if labels are provided
